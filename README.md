...
This toolchain is built on Ubuntu 20.04.1 LTS, which uses glibc 2.31. Compatibility with older distributions cannot be guaranteed. Other libc implementations (such as musl) are not supported.
...
```bash
export PATH="$HOME/toolchains/Alicization-clang/bin:$PATH"
```
...
- If your kernel has a 32-bit vDSO and Kernel Linux version ≥ 4.19: `CROSS_COMPILE_COMPAT=arm-linux-gnueabi-`
...
```bash
$ ${CROSS_COMPILE}ld -v
GNU ld (GNU Binutils) 2.36.50.20210119
```
